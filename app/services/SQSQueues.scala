...
case class SQSMessage[A](id: MessageId, get: A, handle: ReceiptHandle)
...
case class JsonMessageQueue[A](client: AmazonSQSAsyncClient, queueUrl: String)(implicit executionContext: ExecutionContext)
  ...
  def receive(request: ReceiveMessageRequest)(implicit reads: Reads[A]): Future[Seq[SQSMessage[A]]] = {
  ...
  def receiveOne(request: ReceiveMessageRequest)(implicit reads: Reads[A]): Future[Option[SQSMessage[A]]] = {
